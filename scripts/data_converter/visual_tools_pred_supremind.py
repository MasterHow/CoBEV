...
def kitti_visual_tool(data_root, demo_dir, label_path):
    ...
    for i in range(len(image_ids)):
        ...
        image = draw_3d_box_on_image_supremind(image, label_2_file, P2, denorm)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_root", type=str,
                        default="/workspace/mnt/storage/pangchengshan/exp/dataset/dair-v2x-i-kitti",
                        help="Path to Dataset root in KITTI format")
    parser.add_argument("--demo_dir", type=str,
                        default="",
                        help="Path to demo directions")
    parser.add_argument('--pred_dir', type=str, default='', help='path to predictions')
    args = parser.parse_args()
    if not args.demo_dir:
        args.demo_dir = args.pred_dir + '_vis'
    os.makedirs(args.demo_dir, exist_ok=True)
    kitti_visual_tool(args.data_root, args.demo_dir, args.pred_dir)