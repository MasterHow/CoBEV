...
from models.cobev import CoBEV
...
data_root = "/workspace/mnt/storage/shihao/Data-Yara/BEV_Det/public_dataset/dataset/dair-v2x-i-kitti/"
gt_label_path = "/workspace/mnt/storage/shihao/Data-Yara/BEV_Det/public_dataset/dataset/dair-v2x-i-kitti/training/label_2"
...
backbone_conf = {
    ...
    'depth_height_net_conf': dict(in_channels=512, mid_channels=512)
}
ida_aug_conf = {
    'final_dim':
    final_dim,
...
bbox_coder = dict(
    ...
    post_center_range=[0.0, -61.2, -10.0, 224.8, 61.2, 10.0],
    max_num=500,
    score_threshold=0.1,
    out_size_factor=4,
    voxel_size=[0.1, 0.1, 8],
    pc_range=[0, -51.2, -5, 204.8, 51.2, 3],
    code_size=9,
)

train_cfg = dict(
    point_cloud_range=[0, -51.2, -5, 204.8, 51.2, 3],
    grid_size=[2048, 1024, 1],
    voxel_size=[0.1, 0.1, 8],
    out_size_factor=4,
    dense_reg=1,
...
test_cfg = dict(
    post_center_limit_range=[0.0, -61.2, -10.0, 224.8, 61.2, 10.0],
    max_per_img=500,
    max_pool_nms=False,
...
class CoBEVLightningModel(LightningModule):
    ...
    def __init__(self,
        ...
        self.model = CoBEV(self.backbone_conf, self.head_conf)
        self.mode = 'valid'
    ...
    def configure_optimizers(self):
        ...
        scheduler = MultiStepLR(optimizer, [50, 120])
...
def main(args: Namespace) -> None:
    ...
    model = CoBEVLightningModel(**vars(args))
    checkpoint_callback = ModelCheckpoint(dirpath='/workspace/mnt/storage/shihao/Data-Yara/BEV_Det/cobev_3d_lss_r101_864_1536_512x256.DAIRI/checkpoints', filename='{epoch}', every_n_epochs=5, save_last=True, save_top_k=-1)
    trainer = pl.Trainer.from_argparse_args(args, callbacks=[checkpoint_callback])
    if args.evaluate:
        if os.path.isdir(args.ckpt_path):
            for ckpt_name in os.listdir(args.ckpt_path):
                model_pth = os.path.join(args.ckpt_path, ckpt_name)
                model = CoBEVLightningModel.load_from_checkpoint(checkpoint_path=model_pth,
                                                                     strict=False)  # 加载 checkpoint
                # 重新设置路径配置，防止被 checkpoint 覆盖
                model.data_root = data_root  # 保持你自定义的路径
                model.evaluator.data_root = data_root
                model.evaluator.gt_label_path = gt_label_path  # 保持自定义的 gt_label_path
                trainer.test(model)
        else:
            model_pth = args.ckpt_path
            trainer.test(model, ckpt_path=model_pth)
    else:
        backup_codebase(os.path.join('/workspace/mnt/storage/shihao/Data-Yara/BEV_Det/cobev_3d_lss_r101_864_1536_512x256.DAIRI', 'backup'))
        trainer.fit(model)
...
def run_cli():
    ...
    parser = CoBEVLightningModel.add_model_specific_args(parent_parser)
    parser.set_defaults(
        profiler='simple',
        deterministic=False,
        max_epochs=300,
        ...
        precision=32,
        default_root_dir='/workspace/mnt/storage/shihao/Data-Yara/BEV_Det/cobev_3d_lss_r101_864_1536_512x256.DAIRI')
