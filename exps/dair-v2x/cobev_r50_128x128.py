...
train_cfg = dict(
    ...
    grid_size=[512, 512, 1],    # 128和256的区别在于grid size bev网格的定义
    voxel_size=[0.2, 0.2, 8],
    out_size_factor=4,
    dense_reg=1,
...
class CoBEVLightningModel(LightningModule):
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.AdamW(self.model.parameters(),
                                      lr=lr,
                                      weight_decay=1e-7)
        scheduler = MultiStepLR(optimizer, [19, 23])    # hd3d的优化器也有所不同
...
def main(args: Namespace) -> None:
    ...
    # main里的ckpt路径需要修改
    checkpoint_callback = ModelCheckpoint(dirpath='./outputs/cobev_lss_r50_864_1536_128x128/checkpoints', filename='{epoch}', every_n_epochs=5, save_last=True, save_top_k=-1)
...
def run_cli():
    ...
    parser.set_defaults(
        ...
        precision=32,
        default_root_dir='./outputs/cobev_r50_864_1536_128x128')
