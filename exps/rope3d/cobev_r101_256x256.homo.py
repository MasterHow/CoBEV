...
class CoBEVLightningModel(LightningModule):
    ...
    def configure_optimizers(self):
        ...
        #scheduler = MultiStepLR(optimizer, [80, 150])
        scheduler = MultiStepLR(optimizer, [50, 120])
        #scheduler = MultiStepLR(optimizer, [100, 200])
    ...
    def train_dataloader(self):
        train_dataset = NuscMVDetDataset(
            ...
            info_path=os.path.join(data_root, 'rope3d_12hz_infos_train.homologous.pkl'),
            is_train=True,
    ...
    def val_dataloader(self):
        val_dataset = NuscMVDetDataset(
            ...
            classes=self.class_names,
            data_root=self.data_root,
            info_path=os.path.join(data_root, 'rope3d_12hz_infos_val.homologous.pkl'),
            is_train=False,
            img_conf=self.img_conf,
            num_sweeps=self.num_sweeps,
            sweep_idxes=self.sweep_idxes,
...
def main(args: Namespace) -> None:
    ...
    checkpoint_callback = ModelCheckpoint(dirpath='/workspace/mnt/storage/shihao/cobev_lss_r101_864_1536_256x256.rope3d.homologous/checkpoints', filename='{epoch}', every_n_epochs=5, save_last=True, save_top_k=-1)
    trainer = pl.Trainer.from_argparse_args(args, callbacks=[checkpoint_callback])
    if args.evaluate:
        if os.path.isdir(args.ckpt_path):
            #f = open('./eval_orders.txt', 'w')
            #for ckpt_name in os.listdir(args.ckpt_path):
            for ii in range(1, 110, 2):
                ckpt_name = 'epoch='+str(ii)+'.ckpt'
                #f.write(ckpt_name+'\n')
        ...
        if False:
            model = model.load_from_checkpoint('/workspace/mnt/storage/shihao/cobev_lss_r101_864_1536_256x256.epoch150.rope3d.homologous.aug_update/checkpoints/epoch=34.ckpt')
        backup_codebase(os.path.join('/workspace/mnt/storage/shihao/cobev_lss_r101_864_1536_256x256.rope3d.homologous', 'backup'))
        trainer.fit(model)
...
def run_cli():
    ...
    parser.set_defaults(
        ...
        precision=32,
        default_root_dir='/workspace/mnt/storage/shihao/cobev_lss_r101_864_1536_256x256.rope3d.homologous')
