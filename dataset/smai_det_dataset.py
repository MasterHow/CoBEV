...
def point_depth2height(point_depth, K, RT):
    point_uvd = np.stack((point_depth[:,2]*point_depth[:,0], point_depth[:,2]*point_depth[:,1], point_depth[:,2]), 0)  # 3xN
    point_cam = np.linalg.inv(K).dot(point_uvd)  # 3xN
    point_cam_1 = point_cam[0] * 0.0 + 1.0
    point_cam_1 = np.expand_dims(point_cam_1, 0)
    point_cam = np.concatenate((point_cam, point_cam_1), 0)  # 4xN
    RT_inv = np.linalg.inv(RT)
    point_lidar = RT_inv.dot(point_cam)
    point_height = np.stack((point_depth[:,0], point_depth[:,1], point_lidar[2]), 1)
    return point_height
...
class NuscMVDetDataset(Dataset):
    ...
    def sample_bda_augmentation(self):
        ...
        if not self.is_train or True:
            ...
            rotate_bda = np.random.uniform(*bda_aug_conf['rot_lim'])
            scale_bda = np.random.uniform(*bda_aug_conf['scale_lim'])
            flip_dx = np.random.uniform() < bda_aug_conf['flip_dx_ratio']
            flip_dy = np.random.uniform() < bda_aug_conf['flip_dy_ratio']
    ...
    def get_image(self, cam_infos, cams):
        ...
        for cam in cams:
            ...
            for sweep_idx, cam_info in enumerate(cam_infos):
                img = Image.open(
                    os.path.join(self.data_root, 'image_2', cam_info[cam]['filename']))
                ...
                if self.return_depth and sweep_idx == 0:
                    ...
                    point_depth = np.fromfile(os.path.join(
                        self.data_root, 'depth_gt', f'{file_name}.bin'),
                                              dtype=np.float32,
                                              count=-1).reshape(-1, 3)
                    point_height = point_depth2height(point_depth, cam_info[cam]['calibrated_sensor']['camera_intrinsic'], sweepego2sweepsensor.numpy())
                    point_depth_augmented = depth_transform(
                        point_depth, resize, self.ida_aug_conf['final_dim'],
                        crop, flip, rotate_ida)
                    point_height_augmented = depth_transform(point_height, resize, self.ida_aug_conf['final_dim'], crop, flip, rotate_ida)
    ...
    def get_gt(self, info, cams):
        ...
        for ann_info in info['ann_infos']:
            ...
            if (map_name_from_general_to_detection[ann_info['category_name']]
                ...
                continue
            '''
            if ann_info['category_name'] not in self.classes:
                continue
            box = Box(
            ...
            '''
            gt_labels.append(self.classes.index(ann_info['category_name']))
