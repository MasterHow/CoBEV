...
#from layers.backbones.fusion_lut import iAFF
from layers.backbones.fusion_3d import openocc_voxel_pooling, VisFuser, AddFuser, CatFuser, ChannelFusion, CFSFusion, CBAMFusionV2, AxialAttFusion

__all__ = ['LSSFPNHD3DAblations']
...
class DepthHeightNet(nn.Module):
    def __init__(self, in_channels, mid_channels, context_channels,
                 depth_channels, height_channels):
        super(DepthHeightNet, self).__init__()
...
class LSSFPNHD3DAblations(nn.Module):
    def __init__(self, x_bound, y_bound, z_bound, d_bound, height_bound, final_dim,
                 downsample_factor, output_channels, img_backbone_conf,
                 img_neck_conf, depth_height_net_conf, fusion_method):
        ...
        super(LSSFPNHD3DAblations, self).__init__()
        ...
        self.depth_height_net = self._configure_depthheight_net(depth_height_net_conf)
        #self.depth_height_feature_fuse = iAFF(channels=80)
        if fusion_method=='add':
            self.depth_height_feature_fuse = AddFuser(80, 80)
        elif fusion_method=='cat':
            self.depth_height_feature_fuse = CatFuser(80, 80)
        elif fusion_method=='channel':
            self.depth_height_feature_fuse = ChannelFusion(80, 80)
        elif fusion_method=='cbam':
            self.depth_height_feature_fuse = CFSFusion(80, 80)
        elif fusion_method=='cbamv2':
            self.depth_height_feature_fuse = CBAMFusionV2(80, 80)
        elif fusion_method=='axial':
            self.depth_height_feature_fuse = AxialAttFusion(80, 80)

        self.img_neck.init_weights()
        self.img_backbone.init_weights()

    def _configure_depthheight_net(self, depth_height_net_conf):
        return DepthHeightNet(
            depth_height_net_conf['in_channels'],
            depth_height_net_conf['mid_channels'],
    ...
    def create_frustum_height(self):
        ...
        # DID
        alpha = 1.5
        d_coords = np.arange(self.height_bound[2]) / self.height_bound[2]
        d_coords = np.power(d_coords, alpha)
        d_coords = self.height_bound[0] + d_coords * (self.height_bound[1] - self.height_bound[0])
    ...
    def _forward_depth_height_net(self, feat, mats_dict):
        return self.depth_height_net(feat, mats_dict)
    ...
    def _forward_single_sweep(self,
        ...
        depth_height_feature = self._forward_depth_height_net(
        ...
        depth = depth_height_feature[:, :self.depth_channels]
        depth_softmax = F.softmax(depth, 1)
        height = depth_height_feature[:, self.depth_channels:self.depth_channels+self.height_channels].softmax(1)
        img_feat_with_depth = depth_softmax.unsqueeze(
            1) * depth_height_feature[:, self.depth_channels+self.height_channels:(
        ...
        feature_map_height = voxel_pooling(geom_xyz_height, img_feat_with_height.contiguous(),
                                   self.voxel_num.cuda())
        '''
        feature_map_depth = openocc_voxel_pooling(geom_xyz_depth, img_feat_with_depth.contiguous(), self.voxel_size, self.voxel_coord, self.voxel_num)
        feature_map_height = openocc_voxel_pooling(geom_xyz_height, img_feat_with_height.contiguous(), self.voxel_size, self.voxel_coord, self.voxel_num)
        #print('debug1:', feature_map_depth.size(), feature_map_height.size())
        feature_map = self.depth_height_feature_fuse(feature_map_depth, feature_map_height)
        #print('debug2:', feature_map.size())
        if is_return_height:
