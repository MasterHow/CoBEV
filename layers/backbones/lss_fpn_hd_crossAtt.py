...
#from layers.backbones.cbam import CBAM
...
class CrossViewTransformer(nn.Module):
    ...
    def forward(self, front_x, cross_x):
...
class CrossAttFusion(nn.Module):
    def __init__(self, in_dim=80):
        ...
        self.crossview = CrossViewTransformer(in_dim)
        
    def forward(self, x1, x2):
        out = self.crossview(x1, x2)
        return out
...
class LSSFPNHDCA(nn.Module):
    def __init__(self, x_bound, y_bound, z_bound, d_bound, height_bound, final_dim,
        ...
        super(LSSFPNHDCA, self).__init__()
        ...
        self.depth_height_net = self._configure_height_net(depth_height_net_conf)
        #self.depth_height_feature_fuse = iAFF(channels=80)
        #self.depth_height_feature_fuse = CBAM(80*2)
        self.depth_height_feature_fuse = CrossAttFusion(in_dim = 80)

        self.img_neck.init_weights()
        self.img_backbone.init_weights()

    def _configure_height_net(self, depth_height_net_conf):
    ...
    def _forward_single_sweep(self,
        ...
        #print('debug1:', geom_xyz_depth.size(), geom_xyz_height.size(), img_feat_with_depth.size(), img_feat_with_height.size())
        ...
        feature_map_height = voxel_pooling(geom_xyz_height, img_feat_with_height.contiguous(),
                                   self.voxel_num.cuda())
        #print('debug2:', geom_xyz_depth.size(), geom_xyz_height.size(), feature_map_depth.size(), feature_map_height.size())
