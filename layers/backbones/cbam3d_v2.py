...
class CBAM3d_v2(nn.Module):
    def __init__(self, gate_channels, reduction_ratio=5, pool_types=['avg', 'max']):
        super(CBAM3d_v2, self).__init__()
        self.ChannelGate3d = ChannelGate3d(gate_channels, reduction_ratio, pool_types)
        self.SpatialGate3d = SpatialGate3d()
        self.bev_channels = gate_channels//2
    def forward(self, img_voxel_feats, pts_voxel_feats):
        ...
        cat_feats1 = w1 * cat_feats
        w2 = self.SpatialGate3d(cat_feats1)
        cat_feats2 = w2 * cat_feats1
        out_feats = cat_feats2[:,:self.bev_channels] + cat_feats2[:,self.bev_channels:]
        return out_feats